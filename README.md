...
| `packages/CharacterEngine` | `@weaver/character-engine` | **Deterministic player-character facts and core resolution.** Owns Body/Agility/Mind/Presence, ability modifiers, `d20 + modifier + optional proficiency` checks, AC from Agility plus caller-supplied armor, HP/conditions/dying, campaign day counter, and **long-rest full recovery** (`longRest` / `previewLongRest`). Known action *ids* live here; ActionEngine owns definitions. No Electron, no LLM invention. |
...
| `packages/QuestEngine` | `@weaver/quest-engine` | **Deterministic world/campaign quest catalog.** Seeds world quest instances bound to peer ids (regions/places/NPCs/items); list/get/portability. CharacterEngine keeps the per-PC quest log; DMEngine orchestrates seed timing / offers; NarrationEngine invents prose. LLM-free. |
