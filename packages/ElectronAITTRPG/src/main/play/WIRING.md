...
2. That factory calls `openCampaignSession` (or reuses `getActiveCampaignSession` for the same id) and hydrates durable autosave JSON into CharacterEngine.
...
4. Successful turns also write autosave snapshots under `{campaign}/data/autosaves/` via `writeDurableAutosave`.
