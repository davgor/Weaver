...
- [ ] NPC record has an optional portrait reference field, populated asynchronously/non-blocking after construction
- [ ] Portrait generation is gated by the campaign's generative-tokens flag — off by default unless the campaign opts in
- [ ] Companion portraits call the exact same hook as NPC portraits rather than a parallel pipeline (matches AI-DND-Matrix's `139` reusing `122`'s pipeline)
- [ ] Failure to generate a portrait never blocks NPC construction or play — it degrades to "no portrait" silently
