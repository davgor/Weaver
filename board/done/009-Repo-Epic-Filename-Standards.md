...
- [x] This ticket itself uses the new filename pattern (`009-Repo-Epic-Filename-Standards.md` — already compliant); `npm test` / `lint` / `build` / `deadcode` / `act` still pass (docs/skills only). Verified via local gates + `npm run ci:act` (`pr-checks.yml` + `deadcode.yml` both `🏁 Job succeeded`).
