...
- [x] Bestiary species/variant records with stat blocks (abilities, HP via the shared hit-die formula, damage types dealt/resisted)
- [x] Seed catalog covers at least three species at launch (matching AI-DND-Matrix's minimum bar) with room to grow without a schema change
- [x] Package scaffolded matching sibling engines, added to root README package table and `build:engines`
- [x] Catalog `hp` fields are reference-only; hydration always recomputes via `023-CharacterEngine-Hp-Model`'s function
- [x] This package's consumption of `023`/`024`'s CharacterEngine APIs (HP hydration, damage-type taxonomy) is covered by a `*.contract.test.ts` here against CharacterEngine's real published API
