...
- [x] Five damage types with resistance/vulnerability multipliers, unit-tested
- [x] `CONDITION_EFFECTS`-equivalent table: Prone, Stunned, Poisoned, Restrained, Unconscious, each with its concrete mechanical effect (disadvantage, auto-fail Body/Agility saves, `canAct` gating, etc.)
- [x] Dying-save protocol is a distinct state machine from "Unconscious" — Unconscious does not auto-fail dying saves
- [x] At 0 HP, a character enters Unconscious + starts dying saves; only losing that sequence triggers death-mode resolution (unless story-driven death is explicitly flagged elsewhere)
- [x] All condition/damage-type logic is pure and unit-tested with no Electron/LLM imports
