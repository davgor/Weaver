...
For engine packages under `packages/` (`CombatEngine`, `ActionEngine`, `WorldEngine`, `RegionalEngine`, `CivilizationEngine`, `DungeonEngine`, `CharacterEngine`, `ItemEngine`, `NPCEngine`, `EnemyEngine`, `NarrationEngine`, `DMEngine`, `LLMEngine`), Electron IPC/admin logic, and any logic with testable behavior:
