...
For engine packages under `packages/` (`CombatEngine`, `WorldEngine`, `RegionalEngine`, `CivilizationEngine`, `CharacterEngine`, `ItemEngine`, `NPCEngine`, `EnemyEngine`, `NarrationEngine`, `DMEngine`, `LLMEngine`), Electron IPC/admin logic, and any logic with testable behavior:
