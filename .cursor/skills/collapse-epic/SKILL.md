...
When every sub-ticket `XXXX.M` of an epic is in `board/done/` and the epic index file `XXXX-PACKAGE_NAME-Summary.md` has also been moved there, the per-sub-ticket files are now pure history — their content belongs inside the epic file instead of as 5-25 separate files cluttering `board/done/`. This skill folds them in and removes the duplicates.

## 1. Confirm the epic is actually fully done

- Resolve the epic number (zero-padded to 3 digits, e.g. `4` -> `004`).
- Confirm the epic index file `board/done/XXXX-PACKAGE_NAME-Summary.md` (glob `XXXX-*.md`) exists.
- List every `board/done/XXXX.*.md` file — these are the children to fold in.
...
For the epic file, append a `## Sub-tickets` section (create it if missing) after its existing content. For each child file, in ascending `XXXX.M` order:
...
Once every child's content is merged into the epic file, delete the individual `board/done/XXXX.M-*.md` files (`git rm` if the repo already tracks them, plain delete otherwise — either is fine pre-commit, but prefer `git rm` so the removal is staged cleanly alongside the edited epic file).
...
- Confirm `board/done/` no longer has any `XXXX.M-*.md` files for this epic — only the single `XXXX-PACKAGE_NAME-Summary.md`.
